...
var options = {
  ...
  errorPhrases: ["Liquid error:", "Out of stock!"],
  ...
  // make sure you edit this before using the script! Can add or remove these as necessary. 
...
function main() {
  sendSummary( processClientAccount() )
}
...
function sendSummary(results) {
  ...
  var emailBody = "";
  emailBody += results.text;
  totErrs += results.data.badUrls.length;
  totWarns += results.data.warnUrls.length;
  totContentErrs += results.data.onPageErrors.length;
  if (totErrs > 0) {
...
function getRow(clientName, results) {
  ...
  if (the_results.length == 0) {
    the_results = "No errors or warnings to report."
  }
